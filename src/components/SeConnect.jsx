...
const Login = () => {
  ...
  const SignUp = async () => {
    ...
    setIsLoading(true);
    try {
      const res = await API.post("/SignUp", { email, phoneNumber, password });
      toast.success("Account created!");
      // ✅ Handle { success, data: { id, email, role } } shape
      ...
      >
        ...
        <motion.div layout>
          ...
          <div style={{ position: "relative", width: "100%" }}>
            ...
            />
            {showpassword ? (
              <EyeOff id='MobileIC1' onClick={() => setShowPassword(!showpassword)} size={19} className="eye-icon" />
            ) : (
              <Eye id='MobileIC1' onClick={() => setShowPassword(!showpassword)} size={19} className="eye-icon" />
            )}
          </div>
          ...
          >
            {isLoading ? (
              <span style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }}>
                <span style={{ width: '16px', height: '16px', border: '2px solid #ffffff', borderTop: '2px solid transparent', borderRadius: '50%', animation: 'spin 0.8s linear infinite', display: 'inline-block' }}></span>
    ...
    <div className='SeConnect'>
      ...
      <div className='MobileConnect'>
        ...
        <div className='ImagesConnect'>
          <motion.img src={mobile1} className='ImagesConnect1' alt='' initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.2 }} />
          <motion.img src={mobile4} className='ImagesConnect2' alt='' initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.4 }} />
          <motion.img src={mobile2} className='ImagesConnect2' alt='' initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.6 }} />
          <motion.img src={mobile3} className='ImagesConnect3' alt='' initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.8 }} />
        </div>
      </div>
    </div>
  );
