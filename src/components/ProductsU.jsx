...
const Index = () => {
  ...
  const normalizeSizes = (raw) => {
    ...
    try {
      if (Array.isArray(raw)) {
        if (typeof raw[0] === "string" && raw[0].startsWith("[")) return JSON.parse(raw[0]);
        return raw;
      }
  ...
  const availableFilters = useMemo(() => {
    ...
    scopedProducts.forEach((product) => {
      ...
      sizes.forEach((sz) => {
        if (sz) sizeMap.set(sz, (sizeMap.get(sz) || 0) + 1);
      });
    });
    return {
  ...
  const filteredProducts = useMemo(() => {
    ...
    if (Array.isArray(value) && value.length === 2) {
      filtered = filtered.filter(p => typeof p?.price === "number" && value[0] <= p.price && p.price <= value[1]);
    }
    if (selectedColor) {
      filtered = filtered.filter(p => Array.isArray(p?.images) && p.images.some(img => img.color?.toLowerCase() === selectedColor.toLowerCase()));
    }
    if (selectedSize) {
      filtered = filtered.filter(p => normalizeSizes(p.size).includes(selectedSize));
    }
    return filtered;
  ...
  const getImageByColor = (product, color, index = 0) => {
    ...
    if (fallback?.urls?.[index]) return formatImageUrl(fallback.urls[index]);
    return '';
  };
  ...
  useEffect(() => {
    const { parentCategoryId: newParent, subcategoryId: newSubcat } = location.state || {};
    if (newParent) setSubcatSelected(newSubcat || null);
  }, [location.state]);
  ...
  return (
    <div style={{ width: '100%', height: "auto" }}>
      ...
      {FunnelIC && <div onClick={() => setFunnel(false)} className='overflow-2'></div>}
      ...
      <div className='CategoriesPU'>
        ...
        <div className='SubcategorisPU'>
          <div
            style={{ backgroundColor: !subCatSelected ? 'black' : 'white', color: !subCatSelected ? 'white' : '' }}
            onClick={() => { setSubcatSelected(null); setSelectedColor(null); setSelectedSize(null); }}
            className='subcatPU'
          >
            See-All
          </div>
          {subcategories.filter(subcat => subcat.genre === activeGenre).map(Subcat => (
            <div
              key={Subcat._id}
              style={{ backgroundColor: Subcat._id === subCatSelected ? 'black' : 'white', color: Subcat._id === subCatSelected ? 'white' : '' }}
              onClick={() => { setSubcatSelected(Subcat._id); setSelectedColor(null); setSelectedSize(null); }}
              className='subcatPU'
            >
              {Subcat.name}
            </div>
        ...
        {FunnelIC && (
          ...
          >
            ...
            <div className="filter-block">
              <h4>Price</h4>
              <Slider value={value} onChange={handleChange} color='black' valueLabelDisplay="auto" min={0} max={300} />
                ...
                {(availableFilters.colors || [])
                  ...
                  .map((color, i) => (
                    <motion.div
                      ...
                      style={{ backgroundColor: color, border: color === "white" || color === "blanc" ? "1px solid #000" : "none" }}
                      title={color}
                    />
                  ))}
              </div>
            ...
            <motion.button whileTap={{ scale: 0.95 }} className="reset-btn" onClick={() => { setSelectedColor(null); setSelectedSize(null); setValue([0, 300]); }}>
              <Eraser size={16} /> Reset Filters
            </motion.button>
            <motion.button whileTap={{ scale: 0.95 }} className="result-btn" onClick={() => setFunnel(false)}>
              see result
            </motion.button>
          </motion.div>
        )}
          ...
          <div>
            <div className="no-products-container">
              <div className="no-products-content">
                <div className="icon-wrapper"><Package size={64} /></div>
                <h2>No Products Yet</h2>
                <p>We're currently working on adding products to this category.<br />Check back soon for exciting new arrivals!</p>
                <span className="coming-soon">Coming Soon</span>
              </div>
            </div>
            <h1 style={{ fontSize: '19px', fontWeight: 400, marginTop: '0%', marginLeft: '40%' }}>You might be interested in</h1>
            <div className='ProductsU'>
              {products.map((product, index) => {
                ...
                } catch (err) { console.error("Error parsing sizes:", err); }
                return (
                  <div
                    className="productU"
                    style={{ height: "450px" }}
                    key={product._id || index}
                    onClick={() => navigate(`/PorductSelecte/${product._id}`, { state: { parentCategoryId: product.categoryId, subcategoryId: product.subcategoryId, genre: product.genre } })}
                  >
                    <div className="hoverImg" onMouseEnter={() => setHoveredProductId(product._id)} onMouseLeave={() => setHoveredProductId(null)}>
                      <img src={isHovered && secondImg ? secondImg : mainImg} alt={product.name} onError={(e) => (e.target.src = "/default.png")} />
                      {isHovered && (
                        <div className="sizesPU">
                          <h4>Sizes</h4>
                          <div className="sizes-container">
                            {sizes.map((s, indx) => <div key={indx} className="size-box">{s}</div>)}
                          </div>
                        </div>
                      )}
                    </div>
            ...
            return (
              ...
              >
                <div className="hoverImg">
                  ...
                  <AnimatePresence>
                    {isHovered && (
                      <motion.div className="sizesPU" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 10 }} transition={{ duration: 0.3 }}>
                        <h4>Sizes</h4>
                        <div className="sizes-container">
                          {sizes.map((s, indx) => <div key={indx} className="size-box">{s}</div>)}
                        </div>
            ...
            <div className='SearchMobileProductS'>
              {products.map((prod, index) => (
                <div
                  ...
                  style={{ height: "300px" }}
                  onClick={() => navigate(`/PorductSelecte/${prod._id}`, { state: { parentCategoryId: prod.categoryId, subcategoryId: prod.subcategoryId, genre: prod.genre } })}
                >
                  <img src={formatImageUrl(prod?.images?.[0]?.urls?.[0])} alt={prod.name} onError={(e) => (e.target.src = "/default.png")} />
                  <h2>{prod.name}</h2>
      ...
      <footer className="footer">
        <div className="footer-sections">
          ...
          <div className="footer-col">
            ...
            <ul>
              <li>New Arrivals</li>
              <li onClick={() => navigate(`/ProductU/Accessories`, { state: { parentCategoryId: '69160810f85af644092dddb3', subcategoryId: '6916099bf85af644092dddf9', genre: 'men' } })}>Baggy</li>
              <li onClick={() => navigate(`/ProductU/Accessories`, { state: { parentCategoryId: '691607d9f85af644092ddd9f', subcategoryId: '69160937f85af644092dddec', genre: 'men' } })}>Basketball</li>
              <li onClick={() => navigate(`/ProductU/Accessories`, { state: { parentCategoryId: '691607a9f85af644092ddd86', subcategoryId: '691614d6f85af644092de02c', genre: 'men' } })}>similicuir</li>
            </ul>
          </div>
          <div className="footer-col">
